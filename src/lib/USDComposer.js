...
class USDComposer {
  ...
  compose(parsedData, assets = {}, variantSelections = {}, basePath = "") {
    ...
    this.fps =
      rootFields.framesPerSecond || rootFields.timeCodesPerSecond || 30;
  ...
  _buildIndexes() {
    ...
    for (const path in this.specsByPath) {
      ...
      if (spec.specType === SpecType.Prim) {
        ...
        spec.specType === SpecType.Attribute ||
        spec.specType === SpecType.Relationship
      ) {
  ...
  _getVariantPaths(parentPath) {
    ...
    for (const variantSetName of variantSetChildren) {
      ...
      let selectedVariant =
        this.externalVariantSelections[variantSetName] || null;
      ...
      if (!selectedVariant) {
        ...
        selectedVariant = variantSelection
          ? variantSelection[variantSetName]
          : null;
      }
      ...
      if (selectedVariant) {
        const variantPath =
          parentPath + "/{" + variantSetName + "=" + selectedVariant + "}";
  ...
  _resolveReference(refValue, localVariants = {}) {
    ...
    if (referencedData.specsByPath) {
      ...
      const composedGroup = composer.compose(
        referencedData,
        this.assets,
        mergedVariants,
        newBasePath,
  ...
  _collectAttributesFromPath(path, attrs) {
    ...
    for (const [attrName, attrSpec] of attrMap) {
      ...
      if (
        attrName.startsWith("primvars:") &&
        attrSpec.fields?.typeName !== undefined
      ) {
  ...
  _buildMesh(path, spec) {
    ...
    if (geomSubsets.length > 0) {
      geometry = this._buildGeometryWithSubsets(
        attrs,
        geomSubsets,
        hasSkinning,
      );
    ...
    if (hasSkinning) {
      ...
      if (skelBindingSpec) {
        if (
          skelBindingSpec.fields.targetPaths &&
          skelBindingSpec.fields.targetPaths.length > 0
        ) {
  ...
  _getMaterialBindingTarget(primPath) {
    ...
    for (let i = 1; i < parts.length; i++) {
      ...
      for (const vp of variantPaths) {
        const overridePath = relativePath
  ...
  _buildGeometry(path, fields, hasSkinning = false) {
    ...
    const normalIndicesRaw =
      fields["normals:indices"] || fields["primvars:normals:indices"];
    ...
    if (normals && normals.length > 0) {
      ...
      if (normalIndicesRaw && normalIndicesRaw.length > 0 && triPattern) {
        ...
        const triangulatedNormalIndices = this._applyTriangulationPattern(
          normalIndicesRaw,
          triPattern,
        );
        normalData = this._expandAttribute(
          normals,
          triangulatedNormalIndices,
          3,
        );
      ...
      geometry.setAttribute(
        "normal",
        new BufferAttribute(new Float32Array(normalData), 3),
      );
    } else {
    ...
    if (uvs && uvs.length > 0) {
      ...
      if (uvIndices && uvIndices.length > 0 && triPattern) {
        const triangulatedUvIndices = this._applyTriangulationPattern(
          uvIndices,
          triPattern,
        );
      ...
      geometry.setAttribute(
        "uv",
        new BufferAttribute(new Float32Array(uvData), 2),
      );
    }
    ...
    if (uvs2 && uvs2.length > 0) {
      ...
      if (uv2Indices && uv2Indices.length > 0 && triPattern) {
        const triangulatedUv2Indices = this._applyTriangulationPattern(
          uv2Indices,
          triPattern,
        );
      ...
      geometry.setAttribute(
        "uv1",
        new BufferAttribute(new Float32Array(uv2Data), 2),
      );
    }
    ...
    if (hasSkinning) {
      ...
      if (jointIndices && jointWeights) {
        ...
        if (indices && indices.length > 0) {
          skinIndexData = this._expandAttribute(
            jointIndices,
            indices,
            elementSize,
          );
          skinWeightData = this._expandAttribute(
            jointWeights,
            indices,
            elementSize,
          );
  ...
  _buildGeometryWithSubsets(fields, geomSubsets, hasSkinning = false) {
    ...
    const jointIndices = hasSkinning
      ? fields["primvars:skel:jointIndices"]
      : null;
    const jointWeights = hasSkinning
      ? fields["primvars:skel:jointWeights"]
      : null;
    const elementSize = fields["primvars:skel:jointIndices:elementSize"] || 4;
    ...
    const groups = [];
    let currentSubset =
      sortedTriangles.length > 0 ? sortedTriangles[0].subset : -1;
    let groupStart = 0;
    ...
    const { indices: origIndices, pattern: triPattern } =
      this._triangulateIndicesWithPattern(
        faceVertexIndices,
        faceVertexCounts,
        points,
        holeMap,
    ...
    const hasIndexedNormals =
      normals && normalIndicesRaw && normalIndicesRaw.length > 0;
    ...
    const skinIndexData = jointIndices
      ? new Uint16Array(vertexCount * 4)
      : null;
    const skinWeightData = jointWeights
    ...
    for (let i = 0; i < sortedTriangles.length; i++) {
      ...
      for (let v = 0; v < 3; v++) {
        ...
        if (skinIndexData && skinWeightData && jointIndices && jointWeights) {
          for (let j = 0; j < 4; j++) {
            if (j < elementSize) {
              skinIndexData[newIdx * 4 + j] =
                jointIndices[pointIdx * elementSize + j] || 0;
              skinWeightData[newIdx * 4 + j] =
                jointWeights[pointIdx * elementSize + j] || 0;
            } else {
    ...
    if (skinWeightData) {
      geometry.setAttribute(
        "skinWeight",
        new BufferAttribute(skinWeightData, 4),
      );
    }
  ...
  _findUVPrimvar(fields) {
    for (const key in fields) {
      ...
      if (
        ...
        key.endsWith(":elementSize") ||
        key.endsWith(":indices")
      )
        continue;
  ...
  _triangulateIndicesWithPattern(
    ...
    for (let i = 0; i < counts.length; i++) {
      ...
      if (holes && holes.length > 0 && points && points.length > 0) {
        ...
        const triangles = this._triangulateNGonWithHoles(
          faceIndices,
          holeContours,
          points,
        );
  ...
  _triangulateNGon(faceIndices, points) {
    ...
    for (const idx of faceIndices) {
      contour3D.push(
        new Vector3(points[idx * 3], points[idx * 3 + 1], points[idx * 3 + 2]),
      );
    }
  ...
  _triangulateNGonWithHoles(outerIndices, holeContours, points) {
    ...
    const result = [];
    for (const tri of triangles) {
      result.push([allIndices[tri[0]], allIndices[tri[1]], allIndices[tri[2]]]);
    }
  ...
  _getMaterialPath(meshPath, fields) {
    ...
    if (materialBinding) {
      materialPath = Array.isArray(materialBinding)
        ? materialBinding[0]
        : materialBinding;
    }
    ...
    if (!materialPath) {
      ...
      if (materialsInRoot) {
        for (const path of materialsInRoot) {
          if (
            path.startsWith(rootPath + "/Looks/") ||
  ...
  _applyMaterialBinding(mesh, primPath) {
    ...
    let materialPath = null;
    const targetPaths =
      bindingSpec.fields?.targetPaths || bindingSpec.fields?.default;
  ...
  _applyTextureOrValue(
    ...
    if (
      ...
      spec.fields.connectionPaths.length > 0
    ) {
      // For OpenPBR, try all connection paths; for PreviewSurface, just the first
      const paths =
        textureGetter === this._getTextureFromOpenPBRConnection
          ? spec.fields.connectionPaths
  ...
  _applyPreviewSurface(material, shaderPath) {
    ...
    applyTexture("inputs:diffuseColor", "map", SRGBColorSpace, (color) => {
    ...
    const opacityThreshold =
      fields["inputs:opacityThreshold"] !== undefined
        ? fields["inputs:opacityThreshold"]
        : 0.0;

    // Check if opacity is connected to a texture (e.g., diffuse texture's alpha)
    const opacitySpec = getAttrSpec("inputs:opacity");
    const hasOpacityConnection =
      opacitySpec?.fields?.connectionPaths?.length > 0;
    ...
    if (hasOpacityConnection) {
      ...
      const opacity =
        fields["inputs:opacity"] !== undefined ? fields["inputs:opacity"] : 1.0;
  ...
  _applyOpenPBRSurface(material, shaderPath) {
    ...
    // Base color (diffuse)
    applyTexture("inputs:base_color", "map", SRGBColorSpace, (color) => {
      if (Array.isArray(color) && color.length >= 3) {
        material.color.setRGB(color[0], color[1], color[2], SRGBColorSpace);
      }
    });
    ...
    applyTexture("inputs:geometry_normal", "normalMap", NoColorSpace, null);
  }
  ...
  _getTextureFromOpenPBRConnection(connPath) {
    ...
    if (typeName === "NodeGraph") {
      ...
      if (outputSpec?.fields?.connectionPaths?.length > 0) {
        ...
        return this._getTextureFromOpenPBRConnection(
          outputSpec.fields.connectionPaths[0],
        );
      }
    ...
    if (infoId === "MayaND_fileTexture_color4") {
      ...
      if (inColorSpec?.fields?.connectionPaths?.length > 0) {
        return this._getTextureFromOpenPBRConnection(
          inColorSpec.fields.connectionPaths[0],
        );
      }
    ...
    if (infoId && infoId.startsWith("ND_convert_")) {
      ...
      if (inSpec?.fields?.connectionPaths?.length > 0) {
        return this._getTextureFromOpenPBRConnection(
          inSpec.fields.connectionPaths[0],
        );
      }
    ...
    if (infoId === "arnold:bump2d") {
      ...
      if (bumpMapSpec?.fields?.connectionPaths?.length > 0) {
        return this._getTextureFromOpenPBRConnection(
          bumpMapSpec.fields.connectionPaths[0],
        );
      }
    ...
    if (infoId === "arnold:color_correct") {
      ...
      if (inputSpec?.fields?.connectionPaths?.length > 0) {
        return this._getTextureFromOpenPBRConnection(
          inputSpec.fields.connectionPaths[0],
        );
      }
    ...
    if (parentPath) {
      ...
      if (parentSpec) {
        ...
        const parentInfoId =
          parentAttrs["info:id"] || parentSpec.fields["info:id"];
    ...
    if (!assetData) {
      ...
      for (const key in this.assets) {
        if (key.endsWith(baseName) || key.endsWith("/" + baseName)) {
          return this._createTextureFromData(
            this.assets[key],
            textureAttrs,
            transformAttrs,
          );
  ...
  _buildSkeleton(path) {
    ...
    const bindTransforms = this._flattenMatrixArray(
      rawBindTransforms,
      joints.length,
    );
    const restTransforms = this._flattenMatrixArray(
      rawRestTransforms,
      joints.length,
    );
    ...
    if (
      animSourceSpec &&
      animSourceSpec.fields.targetPaths &&
      animSourceSpec.fields.targetPaths.length > 0
    ) {
    ...
    for (const meshData of this.skinnedMeshes) {
      ...
      if (!skeletonData) {
        for (const skelPath in this.skeletons) {
          if (
            skeletonPath &&
            (skeletonPath.includes(skelPath) || skelPath.includes(skeletonPath))
          ) {
      ...
      if (!skeletonData) {
        console.warn(
          "USDComposer: No skeleton found for skinned mesh",
          mesh.name,
        );
        continue;
      }
  ...
  _buildAnimations() {
    ...
    if (transformTracks.length > 0) {
      animations.push(
        new AnimationClip("TransformAnimation", -1, transformTracks),
      );
    }
  ...
  _buildTransformAnimations() {
    ...
    for (const path in this.specsByPath) {
      ...
      const typeName = spec.fields?.typeName;
      if (typeName !== "Xform" && typeName !== "Scope" && typeName !== "Mesh")
        continue;
  ...
  _buildAnimationClip(path) {
    ...
    const translationsAttr = this._getTimeSampledAttribute(
      path,
      "translations",
    );
